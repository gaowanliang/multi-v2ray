...
def alterid():
    ...
    if group == None:
        ...
        if type(group.node_list[client_index]) == Vmess:
            print("{}: {}".format(_("node alterID"),
                                  group.node_list[client_index].alter_id))
...
def dyn_port():
    ...
    if group == None:
        ...
        if choice == 'y':
            newAlterId = input(
                _("please input dyn_port alterID(default 32): "))
...
def new_email():
    ...
    if group == None:
        ...
        print("{}: {}".format(_("node email"),
                              group.node_list[client_index].user_info))
        email = ""
        while True:
...
def port():
    ...
    if group == None:
        ...
        new_port_info = input(
            _("please input new port(support range port(use '-' as separator), all range port can effect):"))
        import re
